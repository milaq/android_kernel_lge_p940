...
static void twl6040_hs_hookkey_detect_work(struct work_struct *work)
{
	...
	if ((hs_jack->jack) && priv->is_hook_enabled) {
		...
		if (is_hookkey_pressed(codec)) {
			...
			hs_hook_report(codec, 0);
			priv->longkey_count = 0;
			dev_info(codec->dev, "%s: hook: shortkey\n", __func__);
		}
	}
}
...
static int p2_cap_put_volsw(struct snd_kcontrol *kcontrol,
...
static const struct snd_kcontrol_new twl6040_snd_controls[] = {
	...
	SOC_DOUBLE_EXT_TLV("Capture Volume",
		TWL6040_REG_MICGAIN, 0, 3, 4, 0, snd_soc_get_volsw, p2_cap_put_volsw, mic_amp_tlv),
#else
