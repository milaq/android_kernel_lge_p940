	...
	unsigned int phy_id)
{
	if (hsi_ll_data.tx_cmd.count >= HSI_LL_MAX_CMD_Q_SIZE) {
		...
		printk("HSI_LL: TX CMD Queue Overflow. %s %d",__func__,__LINE__);
#endif
	...
	*ll_msg_type = ((msg & 0xF0000000) >> 28);
	...
	case HSI_LL_MSG_OPEN_CONN_OCTET:
		...
		*param   = ((msg & 0x00F00000) >> 20) ? (msg & 0x000FFFFF) : (msg & 0x00FFFFFF);
		...
		if (*channel == 0) {
			printk("HSI_LL: Unexpected case. Received CMD = 0x%x. %s %d\n",
...
#if defined (HSI_LL_ENABLE_ERROR_LOG)
		if(ret != 0) {
			printk("HSI_LL:hsi_ll_send_cmd_queue fail for Channel %d.%s %d\n",
					  channel, __func__, __LINE__);
		}
	} else {
		printk("HSI_LL:Invalid command issued for Channel %d. %s %d\n",
				  channel, __func__, __LINE__);
#endif
	}
	spin_unlock_bh(&hsi_ll_if.wr_cmd_lock);
...
static void hsi_ll_read_cb(struct hsi_device *dev, unsigned int size)
{
	...
	if (hsi_ll_data.ch[dev->n_ch].rx.state != HSI_LL_RX_STATE_RX) {
#if defined (HSI_LL_ENABLE_ERROR_LOG)
		printk("HSI_LL: Error. Invalid state for channel %d. %s %d",
		...
		if (hsi_ll_data.ch[dev->n_ch].rx.close_req == TRUE) {
			...
			printk("HSI_LL:Read close requested for ch %d. %s %d\n",
					 dev->n_ch,__func__, __LINE__);
#endif
...
// LGE_UPDATE_END 20120605 seunghwan.jin@lge.com
...
#if defined (HSI_LL_ENABLE_ERROR_LOG)
		printk("HSI_LL: Received Invalid MSG %d from CP for channel %d. %s %d\n",
	...
	case HSI_LL_MSG_BREAK: {
#if defined (HSI_LL_ENABLE_ERROR_LOG)
		printk("HSI_LL:Break received.%s %d\n", __func__, __LINE__);
#endif
...
// LGE_CHANGE [MIPI-HSI] jaesung.woo@lge.com [END]
	...
	case HSI_LL_MSG_ECHO: {
#if defined (HSI_LL_ENABLE_ERROR_LOG)
		printk("HSI_LL:Error.Not supported.%s %d\n", __func__, __LINE__);
#endif
		}
		break;
	case HSI_LL_MSG_OPEN_CONN: {
#if defined (HSI_LL_ENABLE_ERROR_LOG)
		printk("HSI_LL:HSI_LL_MSG_OPEN_CONN Not supported.%s %d.\n",
					__func__, __LINE__);
#endif
		}
		break;
	case HSI_LL_MSG_CONN_CLOSED: {
...
#if defined (HSI_LL_ENABLE_DEBUG_LOG)
			printk("HSI_LL: Data transfer over channel %d completed. %s %d\n",
					  channel, __func__, __LINE__);
#endif
			hsi_ll_cb(channel,
					  HSI_LL_RESULT_SUCCESS,
		...
		default:
#if defined (HSI_LL_ENABLE_ERROR_LOG)
			printk("HSI_LL:Invalid State. %s %d\n", __func__, __LINE__);
#endif
			break;
		}
		}
	...
	case HSI_LL_MSG_ACK: {
...
#if defined (HSI_LL_ENABLE_DEBUG_LOG)
			if (ret!=0) {
				printk("HSI_LL:hsi_write failed for channel %d with error %d. %s %d\n",
						 channel, ret, __func__, __LINE__);
			} else {
				printk("HSI_LL: Transferring data over channel %d. %s %d\n",
						 channel, __func__, __LINE__);
			}
#endif
		...
		default:
#if defined (HSI_LL_ENABLE_ERROR_LOG)
			printk("HSI_LL:Error.Invalid state. Current channel %d state %d. %s %d\n",
	...
	case HSI_LL_MSG_NAK:{
#if defined (HSI_LL_ENABLE_DEBUG_LOG)
		printk("HSI_LL:NAK received for channel %d state %d. retry %d %s %d\n",
		...
		switch(hsi_ll_data.ch[channel].tx.state) {
			case HSI_LL_TX_STATE_OPEN_CONN: {
				...
				if (hsi_ll_data.ch[channel].tx.retry == HSI_LL_MAX_OPEN_CONN_RETRIES) {
					...
					hsi_ll_data.ch[channel].tx.buffer = NULL;
#if defined (HSI_LL_ENABLE_ERROR_LOG)
					printk("HSI_LL: %d retry attempts failed, droping packet for channel %d\n ",
...
#if defined (HSI_LL_ENABLE_DEBUG_LOG)
					printk("HSI_LL: Could not get mem (size=%d)for channel %d, so currently blocking this ch\n",
...
#if defined (HSI_LL_ENABLE_RX_BUF_RETRY_WQ)
#if defined (HSI_LL_ENABLE_ERROR_LOG)
			printk("HSI_LL:Error: Resend open_conn from CP. Current channel %d state %d. %s %d\n",
					channel,hsi_ll_data.ch[channel].rx.state, __func__, __LINE__);
#endif
			hsi_ll_send_command(HSI_LL_MSG_NAK, channel, NULL, HSI_LL_PHY_ID_RX);
...
// IMC_CHANGE [MIPI-HSI] jaesung.woo@lge.com [END]
	...
	default:
#if defined (HSI_LL_ENABLE_ERROR_LOG)
		printk("HSI_LL:Error.Invalid message encountered for channel %d. %s %d\n",
				  channel, __func__, __LINE__);
#endif
		break;
	}
...
#if defined (HSI_LL_ENABLE_DEBUG_LOG)
		printk("HSI_LL: hsi_ll_retry_work - HSI_LL_IF_STATE_ERR_RECOVERY.\n");
#endif
		return;
	}

#if defined (HSI_LL_ENABLE_DEBUG_LOG)
	printk("HSI_LL: retrying Open_connect for ch %d. Size %d. Retry count %d\n",
...
#if defined (HSI_LL_ENABLE_DEBUG_LOG)
	printk("HSI_LL:Write complete for channel %d. %s %d\n",
			  dev->n_ch, __func__, __LINE__);
#endif
...
#if defined (HSI_LL_ENABLE_DEBUG_LOG)
	printk("HSI_LL:Write Complete callback.%s %d\n", __func__, __LINE__);
#endif
...
#if defined (HSI_LL_ENABLE_ERROR_LOG)
		printk("HSI_LL:Cannot create read ctrl Thread.%s %d\n",
				  __func__, __LINE__);
#endif
...
static int hsi_ll_config_bus(void)
{
	...
	if (ret) {
#if defined (HSI_LL_ENABLE_ERROR_LOG)
		printk("HSI_LL:RX config error %d. %s %d\n", ret, __func__, __LINE__);
#endif
	...
	if (ret) {
#if defined (HSI_LL_ENABLE_ERROR_LOG)
		printk("HSI_LL:TX config error %d.%s %d\n", ret, __func__, __LINE__);
#endif
		ret = HSI_LL_RESULT_INIT_ERROR;
		hsi_ll_data.state = HSI_LL_IF_STATE_UN_INIT;
		return HSI_LL_RESULT_ERROR;
...
static int hsi_ll_wr_ctrl_ch_th(void *data)
{
...
#if defined (HSI_LL_ENABLE_DEBUG_LOG)
	printk("HSI_LL:Write thread Started.%s %d\n", __func__, __LINE__);
#endif
	ret = hsi_ll_open(HSI_LL_CTRL_CHANNEL);

	if (ret) {
#if defined (HSI_LL_ENABLE_ERROR_LOG)
		printk("HSI_LL:Error while opening CTRL channel %d.%s %d",
				  ret, __func__, __LINE__);
#endif
		ret = HSI_LL_RESULT_INIT_ERROR;
	...
	if (ret) {
#if defined (HSI_LL_ENABLE_ERROR_LOG)
		printk("HSI_LL: Failed to create RX thread. Initiating HSI Link Layer Shutdown. %s %d\n",
				  __func__, __LINE__);
#endif
		hsi_ll_shutdown();
		return -1;
...
// mo2haewoon.you@lge.com [END]
...
#if defined (HSI_LL_ENABLE_DEBUG_LOG)
				printk("HSI_LL: PSV enable requested.\n");
#endif
#endif
				wait_event_interruptible(hsi_ll_if.msg_avaliable,
										 hsi_ll_if.msg_avaliable_flag == 1);
		...
		if(hsi_ll_data.state == HSI_LL_IF_STATE_ERR_RECOVERY) {
#if defined (HSI_LL_ENABLE_DEBUG_LOG)
			printk("HSI_LL: Stop CMD transfer till recovery completes.\n");
#endif
...
#if defined (HSI_LL_ENABLE_DEBUG_LOG)
			printk("HSI_LL: Requesting AC wake line High.\n");
#endif
...
#if defined (HSI_LL_ENABLE_ERROR_LOG)
		if (ret != 0) {
			printk("HSI_LL: hsi_write(...) failed for ctrl chanel, err=%d. %s %d",
					 ret, __func__, __LINE__);
		}
#endif
		wait_event_interruptible(hsi_ll_if.wr_complete,
...
static int hsi_ll_rd_ctrl_ch_th(void *data)
{
	int ret;
#if defined (HSI_LL_ENABLE_DEBUG_LOG)
	printk("HSI_LL:Read thread Started.%s %d\n", __func__, __LINE__);
#endif

	while(1) {
		if (hsi_ll_data.state == HSI_LL_IF_STATE_ERR_RECOVERY) {
#if defined (HSI_LL_ENABLE_DEBUG_LOG)
			printk("HSI_LL: Stop CMD reception till recovery completes.\n");
#endif
...
#if defined (HSI_LL_ENABLE_ERROR_LOG)
		if (ret) {
			printk("HSI_LL: hsi_read(...) failed for ctrl chanel, err=%d. %s %d",
					  ret, __func__, __LINE__);
		}
#endif
		wait_event_interruptible(hsi_ll_if.rd_complete,
...
int hsi_ll_write(int channel, unsigned char *buf, unsigned int size)
{
...
#if defined (HSI_LL_ENABLE_ERROR_LOG)
		printk("HSI_LL: write attempted for invalid channel %d. %s %d\n",
				  channel, __func__, __LINE__);
#endif
...
#if defined (HSI_LL_ENABLE_ERROR_LOG)
		printk("HSI_LL: write failed as channel %d is not opened. %s %d\n",
				  channel, __func__, __LINE__);
#endif
		goto quit_write;
	}
...
#if defined (HSI_LL_ENABLE_ERROR_LOG)
		printk("HSI_LL: write failed for channel %d due to invalid params. %s %d\n",
				  channel, __func__, __LINE__);
#endif
		goto quit_write;
	}
	...
	if ((hsi_ll_data.ch[channel].tx.state == HSI_LL_TX_STATE_IDLE)  &&
		...
		if (ret != HSI_LL_RESULT_SUCCESS) {
			hsi_ll_data.ch[channel].tx.state = HSI_LL_TX_STATE_IDLE;
#if defined (HSI_LL_ENABLE_ERROR_LOG)
			printk("HSI_LL: write failed for channel %d. %s %d\n",
					  channel, __func__, __LINE__);
#endif
			goto quit_write;
		}
...
#if defined (HSI_LL_ENABLE_ERROR_LOG)
		printk("HSI_LL: write failed .channel %d busy. %s %d\n",
				  channel, __func__, __LINE__);
#endif
		ret = HSI_LL_RESULT_ERROR;
		goto quit_write;
...
int hsi_ll_open(int channel)
{
	...
	if (hsi_ll_data.state == HSI_LL_IF_STATE_ERR_RECOVERY ||
		...
		hsi_ll_data.state == HSI_LL_IF_STATE_UN_INIT) {
#if defined (HSI_LL_ENABLE_ERROR_LOG)
		printk("HSI_LL: Invalid state for channel %d. %s %d\n",
				  channel, __func__, __LINE__);
#endif
		ret = HSI_LL_RESULT_ERROR;
		goto quit_open;
...
#if defined (HSI_LL_ENABLE_ERROR_LOG)
		printk("HSI_LL:Could not open channel %d.%s %d\n",
				  channel, __func__, __LINE__);
#endif
		ret = HSI_LL_RESULT_ERROR;
	} else {
...
#if defined (HSI_LL_ENABLE_ERROR_LOG)
	if (ret) {
		printk("HSI_LL:Error setting AC_WAKE line err=%d. %s %d\n",
				  ret, __func__, __LINE__);
	}
#endif
...
static int hsi_ll_psv_th(void *data)
{
	...
	while(1) {
		...
		do{
			if (hsi_ll_if.psv_event_flag == HSI_LL_PSV_EVENT_PSV_DISABLE) {
#if defined (HSI_LL_ENABLE_DEBUG_LOG)
				printk("HSI_LL:PSV_Enable Revoked.\n");
#endif
				break;
			}
...
// IMC_CHANGE [MIPI-HSI] jaesung.woo@lge.com [START]
			...
			for(channel = 0; channel < HSI_LL_MAX_CHANNELS; channel++) {
				if (hsi_ll_data.ch[channel].tx.state != HSI_LL_TX_STATE_IDLE) {
#if defined (HSI_LL_ENABLE_CRITICAL_LOG)
					printk("HSI_LL:PSV_Enable - Channel[%d] is busy with status %d, retry after a little sleep.\n",
							channel,hsi_ll_data.ch[channel].tx.state);
#endif
			...
			for(channel = 1; channel < HSI_LL_MAX_CHANNELS; channel++) {
				if ((hsi_ll_data.ch[channel].tx.state != HSI_LL_TX_STATE_IDLE) ||
					...
					if((try_count != 0) && (try_count % HSI_LL_PSV_TRY_COUNT_DEBUG_PRINT == 0)) {
						printk("HSI_LL:PSV_Enable - Channel[%d] is busy with TX status %d and RX status %d and retry after a little sleep with try count %d th.\n",
...
#if defined (HSI_LL_ENABLE_DEBUG_LOG)
					printk("HSI_LL: Requesting to set AC wake line to low.\n");
#endif
					hsi_ll_wakeup_cp(HSI_LL_WAKE_LINE_LOW);
					psv_done = 1;
				}
...
static void hsi_ll_timer_bh(unsigned long data)
{
	...
	while(hsi_ll_if.timer_queue_cnt > 0) {
		if (hsi_ll_time_queue[hsi_ll_if.timer_queue_rd].timer_cmd == HIS_LL_TIMER_QUEUE_CMD_START) {
			...
			printk("HSI_LL: State Mis-Match.%s %d\n", __func__, __LINE__);
#endif
...
static void hsi_ll_start_tx_timer(unsigned int channel, unsigned int time_out)
{
	...
	if (hsi_ll_if.timer_queue_cnt > HSI_LL_TIMER_Q_SIZE)
		printk("HSI_LL: Timer Queue Full.%s %d.", __func__, __LINE__);
#endif
...
static void hsi_ll_tx_timer_cb(unsigned long channel)
{
	...
	if (channel >= HSI_LL_MAX_CHANNELS) {
#if defined (HSI_LL_ENABLE_ERROR_LOG)
		printk("HSI_LL:Timer:Invalid channel id.%s %d\n", __func__, __LINE__);
#endif
		return;
	}
	...
	default:
#if defined (HSI_LL_ENABLE_ERROR_LOG)
		printk("HSI_LL:Invalid state.%s %d", __func__, __LINE__);
#endif
		goto quit_tx_timer_cb;
	}
	/* TODO:Complete this */
...
static int hsi_ll_probe_cb(struct hsi_device *dev)
{
...
#if defined (HSI_LL_ENABLE_ERROR_LOG)
		printk("HSI_LL:Invalid channel %d. %s %d.\n",
					dev->n_ch, __func__, __LINE__);
#endif
		return -ENXIO;
	}
...
#if defined (HSI_LL_ENABLE_DEBUG_LOG)
	printk("HSI_LL: probe Channel ID = %d , Dev[%d] = 0x%x . %s %d.\n",
	...
	if (hsi_ll_if.reg_complete_ch_count == HSI_LL_MAX_CHANNELS) {
		hsi_ll_if.reg_complete_flag = 1;
#if defined (HSI_LL_ENABLE_DEBUG_LOG)
		printk("HSI_LL:Registration for %d channels completed.%s %d.\n",
...
int hsi_ll_init(int port, const hsi_ll_notify cb)
{
	...
	if (!hsi_ll_data.initialized) {
		if ((port <= 0)			   ||
			...
			ret = HSI_LL_RESULT_INVALID_PARAM;
#if defined (HSI_LL_ENABLE_ERROR_LOG)
			printk("HSI_LL:Invalid port or callback pointer. %s %d\n",
					__func__, __LINE__);
#endif
			goto quit_init;
		}
...
// LGE_CHANGE [MIPI-HSI] jaesung.woo@lge.com [END]
...
#if defined (HSI_LL_ENABLE_ERROR_LOG)
			printk("HSI_LL:Cannot create Events. %s %d\n",
					__func__, __LINE__);
#endif
			ret = HSI_LL_RESULT_ERROR;
			hsi_ll_data.state = HSI_LL_IF_STATE_UN_INIT;
		...
		if (ret) {
#if defined (HSI_LL_ENABLE_ERROR_LOG)
			printk("HSI_LL:Error while registering with HSI PHY driver err=%d. %s %d\n",
					ret, __func__, __LINE__);
#endif
			ret = HSI_LL_RESULT_INIT_ERROR;
			goto quit_init;
...
#if defined (HSI_LL_ENABLE_ERROR_LOG)
			printk("HSI_LL:Cannot create Timers. %s %d\n",
					__func__, __LINE__);
#endif
			ret = HSI_LL_RESULT_ERROR;
			hsi_ll_data.state = HSI_LL_IF_STATE_UN_INIT;
...
#if defined (HSI_LL_ENABLE_DEBUG_LOG)
		printk("HSI_LL:creating threads. %s %d.\n", __func__, __LINE__);
#endif
...
#if defined (HSI_LL_ENABLE_ERROR_LOG)
			printk("HSI_LL:Cannot create write ctrl Thread. %s %d\n",
					  __func__, __LINE__);
#endif
			ret = HSI_LL_RESULT_ERROR;
			hsi_ll_data.state = HSI_LL_IF_STATE_UN_INIT;
...
#if defined (HSI_LL_ENABLE_ERROR_LOG)
			printk("HSI_LL:Cannot create PSV Thread. %s %d\n",
					  __func__, __LINE__);
#endif
			ret = HSI_LL_RESULT_ERROR;
			hsi_ll_data.state = HSI_LL_IF_STATE_UN_INIT;
...
int hsi_ll_shutdown(void)
{
	...
	if (hsi_ll_data.initialized) {
#if defined (HSI_LL_ENABLE_DEBUG_LOG)
		printk("HSI_LL:HSI Link Layer Shutdown initiated. %s %d\n",
				  __func__, __LINE__);
#endif
...
int hsi_ll_reset(void)
{
	...
	hsi_ll_data.initialized = FALSE;
#if defined (HSI_LL_ENABLE_CRITICAL_LOG)
	printk("HSI_LL: DLP recovery started.\n");
#endif
...
#endif
#if defined (HSI_LL_ENABLE_DEBUG_LOG)
	printk("HSI_LL: Requesting read/write cancel.\n");
#endif
	for(ch_i = 0; ch_i < HSI_LL_MAX_CHANNELS; ch_i++) {
...
#if defined (HSI_LL_ENABLE_DEBUG_LOG)
	printk("HSI_LL: Unblocking all read/write callbacks.\n");
#endif
...
#if defined (HSI_LL_ENABLE_DEBUG_LOG)
	printk("HSI_LL: Resetting HSI PHY Driver.\n");
#endif
...
#if defined (HSI_LL_ENABLE_DEBUG_LOG)
	printk("HSI_LL: Revoking PSV enable.\n");
#endif
...
// LGE_CHANGE [MIPI-HSI] jaesung.woo@lge.com [END]
...
#if defined (HSI_LL_ENABLE_DEBUG_LOG)
	printk("HSI_LL: Reconfiguring HSI Buses.\n");
#endif
	...
	hsi_ll_data.state = HSI_LL_IF_STATE_READY;
#if defined (HSI_LL_ENABLE_DEBUG_LOG)
	printk("HSI_LL: Resuming read/write threads.\n");
#endif
	hsi_ll_if.wr_complete_flag = 2;
	hsi_ll_if.rd_complete_flag = 2;
	wake_up_interruptible(&hsi_ll_if.wr_complete);
...
#if defined (HSI_LL_ENABLE_CRITICAL_LOG)
	printk("HSI_LL: DLP recovery completed.\n");
#endif
...
#if defined (HSI_LL_ENABLE_DEBUG_LOG)
			printk("HSI_LL: Got mem(size=%d), so unblocking ch %d.\n",
						tmp->size,channel);
#endif
